...
setwd("D:/Rishikesh_Data_Science/R_proggrame_coursera/Getting and cleaning data/ASSIGNMENT")
...
#read training data
train_x <- read.table("./Data/Raw_Dataset/UCI HAR Dataset/train/X_train.txt",)
train_y <- read.table("./Data/Raw_Dataset/UCI HAR Dataset/train/y_train.txt",)
sub_train <- read.table("./Data/Raw_Dataset/UCI HAR Dataset/train/subject_train.txt",)

#read test data
test_x <- read.table("./Data/Raw_Dataset/UCI HAR Dataset/test/X_test.txt",)
test_y <- read.table("./Data/Raw_Dataset/UCI HAR Dataset/test/y_test.txt",)
sub_test <- read.table("./Data/Raw_Dataset/UCI HAR Dataset/test/subject_test.txt",)
...
HumanActivity <- rbind(cbind(sub_train,train_x,train_y),cbind(sub_test,test_x,test_y))

#read features and activity label
features <- read.table("./Data/Raw_Dataset/UCI HAR Dataset/features.txt",)
...
ColIntrested <- grepl("Subject|mean|std|Activity",colnames(HumanActivity)) #Logical output so we know which coloumn have mean std and activity(wich is y)
...
#Read Activity Table
activity_labels <- read.table("./Data/Raw_Dataset/UCI HAR Dataset/activity_labels.txt")

HumanActivity$Activity <- factor(HumanActivity$Activity,levels = activity_labels[,1], labels = activity_labels$V2)
...
ColumnNames <- gsub("[Ff]req","Frequency",ColumnNames,ignore.case = "FrequencyDomain")
